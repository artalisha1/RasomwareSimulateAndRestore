...
#Restic Ransomware Simulation & Auto-Recovery Lab — V2 (Baseline Diffing + Periodic Snapshots)
...
* runs a periodic scheduler that re-tags the latest state as a new baseline—and can be stopped cleanly
* keeps new baselines only when real changes are detected (baseline diffing) to avoid redundant snapshots
* simulates a ransomware attack by encrypting docs and tagging an attack snapshot
* automatically finds the latest readable baseline, restores it, verifies key files, and swaps the clean data back into place—removing malicious artifacts and pruning old logs.
...
"""###Return List of Snapshots tagged 'baseline' in Restic Repo"""
...
"""###Print the Difference Between the Snapshots"""
...
* If no baseline exists, take a snapshot
* Otherwise:
  * Record old_id = latest baseline snapshot
  * Run `restic backup --tag baseline` to create new snapshot
  * Let new_id = ID of that new snapshot
  * Call get_diff_files(old_id, new_id):
...
Spawns a background thread that re-tags the latest state as a new baseline every `interval` seconds by calling ensure_baseline_snapshot().
"""
...
* restores to a temporary directory,
* verifies critical files, moves data,
* returns the restored snapshot ID
"""
